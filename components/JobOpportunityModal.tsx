            ...
            return (
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <div>
                            <h3 className="text-lg font-bold text-gray-800">Drafting Email</h3>
                            <p className="text-sm text-gray-500">To: {currentTarget.companyName} | Role: {currentTarget.jobTitle}</p>
                        </div>
                    ...
                    <div className="space-y-6">
                        {results.length > 0 ? results.map((result, index) => (
                            <div key={index} className="bg-gray-50 p-4 rounded-md border">
                                <h3 className="text-lg font-semibold text-gray-800 mb-3">{result.jobTitle}</h3>
                                    ...
                                    {result.companies.map((company, cIndex) => (
                                        <div key={cIndex} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 bg-white rounded-md border">
                                            ...
                                            <button 
                                                ...
                                                className="mt-2 sm:mt-0 flex items-center justify-center px-3 py-1.5 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50"
                                            >
                            ...
                            <p className="text-center text-gray-500">No opportunities found for your query. Try different cities or check your CV details.</p>
                        )}
                    </div>
                )}
            </div>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4 transition-opacity duration-300">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
                <header className="p-4 border-b flex justify-between items-center">
                    <h2 className="text-xl font-bold text-gray-800">Job Opportunity Finder</h2>
                    <button onClick={handleClose} className="text-gray-400 hover:text-gray-600 p-1 rounded-full text-2xl leading-none">&times;</button>
                </header>
